...
contract Casino {
  ...
  struct AcceptedBet {
    ...
    uint acceptedAt;
    uint randomB;
    uint hashB; //hashB to enable #3 protocol
  }   // struct AcceptedBet
  ...
  // Called by sideA to revealA
  function revealA(uint _random) external payable {
    uint _commitment = uint256(keccak256(abi.encodePacked(_random)));
    ...
    proposedBet[_commitment].revealedA = true;

    emit BetRevealedA(_commitment, proposedBet[_commitment].sideA);

  }  // function revealA

  // Called by sideB to reveal B random value and conclude the bet
  function revealB(uint _commitment, uint _randomB) external  {
  
    uint _hashB = uint256(keccak256(abi.encodePacked(_randomB)));

    address payable _sideB = payable(msg.sender);
    address payable _sideA = payable(proposedBet[_commitment].sideA);

    uint _agreedRandom = _randomB ^ proposedBet[_commitment].randomA;
    uint _value = proposedBet[_commitment].value;
    // console.log("randomA: ", proposedBet[_commitment].randomA);
    // console.log("randomB: ", _randomB);
    // console.log("_agreedRandom: ", _agreedRandom);
    // console.log("value: ", _value);

    require(acceptedBet[_commitment].sideB == msg.sender,
      "Not a bet you accepted");
    require(proposedBet[_commitment].accepted && proposedBet[_commitment].revealedA,
      "Bet has not been accepted or revealed A side yet");
    require(acceptedBet[_commitment].hashB == _hashB,
      "wrong B side random number, not match with hashB");
    
    // Pay and emit an event
    if (_agreedRandom % 2 == 0) {
      // sideA wins
      console.log("side A wins : ", _sideA);
      console.log("befor transfer: ", _sideA.balance);
      _sideA.transfer(2*_value);
      console.log("after transfer: ", _sideA.balance);
      emit BetSettled(_commitment, _sideA, _sideB, _value);
    } else {
      // sideB wins
      console.log("side B wins : ",_sideB);
      console.log("befor transfer: ", _sideA.balance);
      _sideB.transfer(2*_value);
      console.log("after transfer: ", _sideA.balance);
      emit BetSettled(_commitment, _sideB, _sideA, _value);      
