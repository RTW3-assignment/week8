...
describe("Casino", async () => {
  ...
  it("Work all the way through (B wins)", async () => {
    ...
    tx = await cA.revealA(valA)
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetRevealedA")      

    // expect(rcpt.events[0].value).to.equal(1e10)   
    // A sends the transaction, so the change due to the
    // bet will only be clearly visible in B

    // console.log("signer[0].address : ", signer[0].address)    
    // now B sends the transaction, so the change due to bet 
    // will only be clearly visible in A
    preBalanceA = await ethers.provider.getBalance(signer[0].address)
    console.log("preBalanceA : ", preBalanceA )    

    tx = await cB.revealB(hashA, valBwin)
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetSettled")

    postBalanceA = await ethers.provider.getBalance(signer[0].address)    
    console.log("postBalanceA : ", postBalanceA)   
  ...
  it("Work all the way through (A wins)", async () => {
    ...
    tx = await cB.revealB(hashA, valBlose)
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetSettled")
